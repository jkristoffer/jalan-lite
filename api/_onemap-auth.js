...
async function getOneMapToken(){
  ...
  if(!email||!password){const error=new Error('OneMap routing is not configured. Add ONEMAP_EMAIL and ONEMAP_PASSWORD in Vercel.');error.code='ONEMAP_NOT_CONFIGURED';throw error}
  ...
  cachedToken=data.access_token;cachedExpiry=Number(data.expiry_timestamp||now+3600);return cachedToken;
}
