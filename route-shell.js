...
function arrivalText(value){return value===0?'Arr':Number.isFinite(value)?`${value} min`:'—'}
...
function legTiming(leg){
 if(leg.mode==='BUS'&&leg.live?.arrivals?.length){const values=leg.live.arrivals.filter(Number.isFinite).slice(0,3);if(values.length)return`<div class="leg-live"><span class="live-dot"></span>${values.map(v=>`<b>${arrivalText(v)}</b>`).join('')}<em>live</em></div>`}
 if(leg.mode==='BUS'&&leg.liveStatus==='loading')return`<div class="leg-live muted">Checking live arrivals…</div>`;
 if(leg.mode==='SUBWAY'&&leg.departureTime)return`<div class="leg-live scheduled"><b>${fmtTime(leg.departureTime)}</b><em>scheduled</em></div>`;
 return'';
}
function routeCard(){
 ...
 if(!itinerary)return`<section class="best-route-card"><div class="route-card-label">Best route</div><h2>Ready to route</h2><p>Route data will load automatically when routing is available.</p></section>`;
 const legs=(itinerary.legs||[]).map(leg=>`<div class="route-leg"><span class="leg-mode ${String(leg.mode||'').toLowerCase()}">${esc(leg.mode==='SUBWAY'?'MRT':leg.mode||'PT')}</span><div><strong>${esc(leg.label||leg.mode||'Travel')}</strong><span>${esc(leg.detail||'')}</span>${legTiming(leg)}</div></div>`).join('');
...
function dashboardView(){
 const itinerary=routeState.data?.itinerary;const hasBus=itinerary?.legs?.some(l=>l.mode==='BUS');const hasMrt=itinerary?.legs?.some(l=>l.mode==='SUBWAY');
 const liveCopy=hasBus?'Bus legs use LTA real-time arrivals at the boarding stop.':hasMrt?'This route has no bus leg to refresh.':'';
 const mrtCopy=hasMrt?' MRT departures are still schedule-based until LTA GTFS-Realtime train updates are connected.':'';
 return`<div class="route-panel">${brand()}<div class="route-header"><div><div class="route-kicker">Saved commute</div><h1>Ready when you are.</h1></div><button class="route-link compact" data-route-action="edit">Edit</button></div>${journeyCard()}${routeCard()}<section class="timing-card"><div class="timing-heading"><div><div class="route-card-label">Timing sources</div><h2>Bus live · MRT scheduled</h2></div><span class="live-state">LTA</span></div><p>${esc((liveCopy+mrtCopy).trim()||'Live timing appears when the calculated route includes public transport.')}</p></section><div class="route-actions"><button class="route-primary" data-route-action="refresh">Refresh route + timings</button><button class="route-link" data-route-action="bus">Open bus arrivals</button><button class="route-link" data-route-action="clear">Remove saved commute</button></div></div>`
}
...
async function refreshPickerLabel(){try{const r=await fetch(`/api/location?lat=${picker.center.lat}&lng=${picker.center.lng}`);const d=await r.json();picker.label=r.ok&&d.label?d.label:'Pinned location'}catch{picker.label='Pinned location'}updatePickerDom()}
...
async function mountMap(){
 const container=document.getElementById('route-map');const fallback=document.getElementById('map-fallback');if(!container||!pickerField)return;if(!window.mapboxgl){if(fallback)fallback.hidden=false;return}
 try{const token=await getMapboxToken();if(!document.getElementById('route-map')||!pickerField)return;mapboxgl.accessToken=token;map=new mapboxgl.Map({container:'route-map',style:'mapbox://styles/mapbox/streets-v12',center:[picker.center.lng,picker.center.lat],zoom:picker.zoom,minZoom:10.5,maxZoom:18.5,maxBounds:[[103.55,1.15],[104.10,1.49]],pitch:0,bearing:0,dragRotate:false,touchPitch:false,attributionControl:true});map.touchZoomRotate.disableRotation();map.addControl(new mapboxgl.NavigationControl({showCompass:false}),'top-right');map.on('load',()=>{mapReady=true;if(fallback)fallback.hidden=true;refreshPickerLabel()});map.on('move',()=>{const c=map.getCenter();picker.center={lat:c.lat,lng:c.lng};picker.zoom=map.getZoom();picker.label='Pinned location';updatePickerDom()});map.on('moveend',refreshPickerLabel);map.on('error',()=>{if(!mapReady&&fallback)fallback.hidden=false})}catch{if(fallback)fallback.hidden=false}
}
async function useManual(){
 const input=document.getElementById('picker-manual-input');const button=shell.querySelector('[data-route-action="use-manual"]');const hint=document.getElementById('picker-manual-hint');const q=String(input?.value||'').trim();if(!q)return;if(button){button.disabled=true;button.textContent='Finding…'}if(hint)hint.textContent='Searching OneMap…';
...
function useMyLocation(){if(!navigator.geolocation)return;const b=shell.querySelector('[data-route-action="my-location"]');if(b){b.disabled=true;b.textContent='Locating…'};navigator.geolocation.getCurrentPosition(pos=>{picker.center={lat:pos.coords.latitude,lng:pos.coords.longitude};picker.zoom=16.5;picker.label='My location';if(map){map.easeTo({center:[picker.center.lng,picker.center.lat],zoom:picker.zoom,duration:350})}else updatePickerDom();if(b){b.disabled=false;b.textContent='◎ My location'}},()=>{if(b){b.disabled=false;b.textContent='Location unavailable'}},{enableHighAccuracy:true,timeout:7000,maximumAge:60000})}

function normalizeItinerary(raw){
 const plan=raw?.plan||raw?.data?.plan;const it=plan?.itineraries?.[0];if(!it)return null;
 const legs=(it.legs||[]).map(leg=>{const mode=String(leg.mode||'').toUpperCase();const from=leg.from?.name||'';const to=leg.to?.name||'';const routeName=leg.routeShortName||leg.route||leg.agencyName||'';const label=mode==='WALK'?`Walk ${fmtDistance(leg.distance)}`:(routeName?`${mode==='SUBWAY'?'MRT':mode} ${routeName}`:(mode==='SUBWAY'?'MRT':mode));return{mode,label,detail:[from,to].filter(Boolean).join(' → '),routeName:String(routeName||''),stopCode:String(leg.from?.stopCode||''),departureTime:Number(leg.startTime||leg.from?.departure||0),live:null,liveStatus:mode==='BUS'?'loading':null}});
